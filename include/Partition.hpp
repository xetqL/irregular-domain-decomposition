...
public:
    ...
    Partition(Point_3 coord, const Box3& domain, const Real* grid_cell_size,
            ...
            : grid_cell_size(grid_cell_size), coord(std::move(coord)),
    ...
    Partition(Point_3 coord, const Real* grid_cell_size,
              Point_3 v1, Point_3 v2, Point_3 v3, Point_3 v4,
              Point_3 v5, Point_3 v6, Point_3 v7, Point_3 v8)
            : grid_cell_size(grid_cell_size), coord(std::move(coord)),
              vertices({std::move(v1), std::move(v2), std::move(v3), std::move(v4),
 ...
 */
    ...
    bool compute_vertex_movement(Real avg_load, Real mu, const Point_3& v, const VertexIndex vid, const std::vector<mesh::Cell<A>>& elements, size_t com, const int MAX_TRIAL = 3){
        ...
        if(communicator.comm_size > 1) {
            ...
            std::vector<Point_3> points; points.reserve(elements.size());
...
#endif
            std::vector<Real> normalized_loads; normalized_loads.reserve(loads.size());
            ...
            auto f1_after = constraint_force(0.0, 0.0, 0.0,
                                             grid_params.get_maximum_simulation_size_x(),
                                             grid_params.get_maximum_simulation_size_y(),
                                             grid_params.get_maximum_simulation_size_z(),
    ...
    std::vector<std::pair<ProcRank, std::vector<DataIndex>>> move_vertices(
        ...
        while(iteration < 8) {
            ...
            if(vertex_trial > 0 && communicator.comm_size > 1) {
                ...
                auto f1_after = constraint_force(0.0, 0.0, 0.0,
                                                 grid_params.get_maximum_simulation_size_x(), grid_params.get_maximum_simulation_size_y(),
                                                 grid_params.get_maximum_simulation_size_z(),
                                                 v, f1);
                int are_all_valid = false;
                for(int trial = 0; trial < MAX_TRIAL; ++trial) {
        ...
        for(int i = 0; i < number_of_neighbors; ++i) {
            ...
            neighborhoods.emplace_back(coord, grid_cell_size, (recv_buff.begin() + i * 24));
        }
    ...
    template<class A>
    std::unordered_map<ProcRank, std::vector<DataIndex>> get_ghost(const std::vector<mesh::Cell<A>>& elements){
        ...
        for(auto neighbor : neighbor_list) ghost_and_destination[neighbor].reserve(1000);

        for(lb::DataIndex i = 0; i < size; ++i) {
            const A &el = elements.at(i);
            const auto point = el.get_center_point();
            auto close_planes = find_planes_closer_than(planes, point, 1.0);
            for(const auto& p : close_planes) {
                auto owner = ranks_per_plane[p];
                if(owner >= 0)
                    ghost_and_destination[owner].push_back(i);
            }
        }
    }
    template<class LoadComputer, class A>
    void move_selected_vertices(int com,
        ...
        std::transform(active_neighbors.begin(), active_neighbors.end(), gid_and_destination.begin(),
                       [](ProcRank i){
            auto vec = std::vector<DataIndex>();
            vec.reserve(2000);
            return std::make_pair(i, vec);
        });
        ...
        while(data_id < nb_elements) {
            if(elements[data_id].type == mesh::REAL_CELL) {
                ...
                if (!is_real_cell) { // it is not in my polygon
                    ...
                    auto lid = mesh::compute_lid(grid_params.get_cell_number_x(),
                            grid_params.get_cell_number_y(),
                            grid_params.get_cell_number_z(), elements[data_id].gid, bbox);


                    new_elements.at(lid).lid = lid;
                    new_elements.at(lid).lid = elements[data_id].gid;
                    new_elements.at(lid).type = elements[data_id].type;
                }
            }
            data_id++;
        }
        ...
        CommunicationDatatype datatype = A::register_datatype();
        {
            ...
            for(auto dest_rank : active_neighbors) {
                ...
                while(i < count) {
                    auto gid = gids_buf[i];
                    auto lid = mesh::compute_lid(grid_params.get_cell_number_x(), grid_params.get_cell_number_y(), grid_params.get_cell_number_z(), gid, bbox); //mesh::compute_lid(domain.size_x, domain.size_y, domain.size_z, gid, bbox);
                    new_elements.at(lid).lid  = lid;
                    new_elements.at(lid).gid  = gid;
                    new_elements.at(lid).type = mesh::REAL_CELL;
                    i++;
                }
            }
            for(auto dest_rank : active_neighbors) {
    ...
    const NumericalType get_vertex_id(const Point_3& v, int N) const {
        ...
        const Real step = grid_params.get_maximum_simulation_size_x() / (vertices_per_row);
    ...
    void dbg_out(){
        ...
        for (auto v : vertices) {
            std::cout << "lb::Point_3(" << v.x() << "," << v.y() << ", "<< v.z() << "),\n";
        }
        std::cout << "};"<< std::endl;
    }
};
