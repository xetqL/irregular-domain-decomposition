...
public:
    virtual void generate_elements(std::vector<elements::Element<N>>* elements, int n,
                                   std::shared_ptr<lennard_jones::RejectionCondition<N>> cond) = 0;
};
...
public:
    ...
    explicit RandomElementsInClustersGenerator(const int max_particles_per_cluster, const int seed = __rd(), const int max_trial = 10000) :
...
public:
    explicit RandomElementsInNClustersGenerator(std::vector<int> clusters, const int seed = __rd(), const int max_trial = 10000) :
    ...
    void generate_elements(std::vector<elements::Element<N>>* elements, const int n,
        ...
        const auto clusters_to_generate = clusters.size();
...
class UniformRandomElementsGenerator : public RandomElementsGenerator<N> {
    ...
    const int max_trial;
public:
    explicit UniformRandomElementsGenerator(int seed, const int max_trial = 10000) : seed(seed), max_trial(max_trial) {}
...
public:
    ...
    void generate_elements(std::vector<elements::Element<N>>* elements, const int n,
        ...
        int number_of_element_generated = 0;
        auto already_generated = elements->size();
